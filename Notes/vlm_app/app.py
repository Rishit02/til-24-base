...
def object_detection_predict(image): # Given an image, this function performs object detection using the SSD300 model and returns boxes
    ...
    weights = torchvision.models.detection.SSD300_VGG16_Weights.DEFAULT # Load the default weights
    ssd_model = torchvision.models.detection.ssd300_vgg16(
        weights=True, box_score_thresh=0.9 # Set the score threshold
    )
...
def clip_predict(image, labels): # Given an image and labels, this function performs object detection and identification using the CLIP model + DETR
